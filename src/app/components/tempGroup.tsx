...
export default function TempGroupWidget({ urls, prefs }: { urls: Urls; prefs: Preferences }) {
  const accountId = 'HCC';
  ...
  useEffect(() => {
    SBDevices.connection$.subscribe({
      next: (status) => setLoadedDevices(SBDevices.transformDeviceStatus(status)),
    });
        ...
        <div className="max-w-7xl mx-auto py-4 px-6 lg:px-8">
          <div className="pt-4 px-4 grid gap-4 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
            {loadedDevices?.map((deviceStatus: DeviceStatus, idx: number) => {
              return <TempWidget key={deviceStatus.deviceId} data={deviceStatus} prefs={prefs}></TempWidget>;
            })}
          </div>
        </div>
      </Suspense>
