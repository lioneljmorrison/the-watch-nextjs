...
const BatteryMeter: React.FC<BatteryMeterProps> = ({ batteryLevel }) => {
  ...
  const batteryTextColor = batteryLevel <= 20 ? 'text-gray-900' : batteryLevel <= 50 ? 'text-gray-900' : 'text-white';
  const batteryWidth = `${batteryLevel}%`;

  return (
    <div className="flex items-center rounded-3xl max-w-12 relative">
      ...
      <span
        className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-xs font-medium ${batteryTextColor}`}
      >
